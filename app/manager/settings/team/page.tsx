...
import { listDevices, summarize } from "@/lib/push";
...
export default async function TeamPage() {
  ...
  const [staff, locations, devices] = await Promise.all([
    listStaff(),
    listAllLocations(),
    listDevices().catch(() => []),
  ]);
  return (
    <div className="mgr-card">
      ...
      <TeamManager initialStaff={staff} locations={locations} currentId={me.id} devices={devices.map(summarize)} />
    </div>
  );
}
