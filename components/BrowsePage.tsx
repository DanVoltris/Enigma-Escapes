...
// The server page (app/(site)/page.tsx) renders this with the day's sessions and
// the room list already loaded, so they show with the page instead of after a
// second round trip. Either can be null if that lookup failed on the server —
// the browser then fetches it the old way.
export default function BrowsePage({
  ...
  const [experiences, setExperiences] = useState<ExperienceSummary[]>(initialExperiences ?? []);
  const [slots, setSlots] = useState<Slot[] | null>(initialSlots);
  // The date the server already loaded. Its first lookup here happens in the
  // background, over the list already on screen: /api/availability is also what
